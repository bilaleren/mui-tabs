...
function findScrollButton(
  ...
  return container.querySelector<HTMLElement>(
    `.${tabsClasses.root} > .tabs-scroll-buttons-${direction}`
  )
}
...
describe('<Tabs />', () => {
  ...
  describe('prop: action', () => {
    it('should be able to access updateIndicator function', () => {
      const tabsActions = React.createRef<TabsActionRefAttributes>()

      render(
        <Tabs value={0} action={tabsActions}>
          <Tab />
          <Tab />
        </Tabs>
      )

      expect(tabsActions.current!.updateIndicator).to.be.a('function')

      tabsActions.current!.updateIndicator()
    })

    it('should be able to access updateScrollButtons function', () => {
      let tabsActions: TabsActionRefAttributes | null
      ...
      expect(tabsActions!.updateScrollButtons).to.be.a('function')

      tabsActions!.updateScrollButtons()
    })
  })
  ...
  describe('prop: scrollButtons', () => {
    ...
    it('should render scroll buttons', () => {
      ...
      expect(
        container.querySelectorAll(`.${tabsClasses.scrollButtons}`)
      ).to.have.lengthOf(2)
    })
    ...
    it('should not hide scroll buttons when allowScrollButtonsMobile is true', () => {
      ...
      expect(
        container.querySelectorAll(`.${tabsClasses.scrollButtonsHideMobile}`)
      ).to.have.lengthOf(0)
    })
  ...
  describe('prop: ButtonComponent', () => {
    it('should set another component', () => {
      const { container } = render(
        <Tabs
          value={0}
          ButtonComponent={({ children }) => (
            <div className="button-component">{children}</div>
          )}
        >
          <Tab />
          <Tab />
      ...
      const tabs = container.querySelectorAll(
        `.${tabsClasses.flexContainer} > .button-component`
      )
